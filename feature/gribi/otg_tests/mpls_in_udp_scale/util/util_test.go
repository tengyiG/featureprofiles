...
const (
	// testDefaultNetworkInstance represents the default network instance for testing
	testDefaultNetworkInstance = "DEFAULT"
)
...
func TestGenerateScaleProfileEntries(t *testing.T) {
	...
	validCfg := &ScaleProfileConfig{
		AddrFamily:          "ipv6",
		NetworkInstanceName: testDefaultNetworkInstance,
		NumPrefixes:         2,
		NumNexthopGroup:     2,
		NumNexthopPerNHG:    2,
	...
	tests := []struct {
	...
	}{
		{
			...
			wantNHs: []fluent.GRIBIEntry{
				fluent.NextHopEntry().
					WithNetworkInstance(testDefaultNetworkInstance).
					WithIndex(1).
